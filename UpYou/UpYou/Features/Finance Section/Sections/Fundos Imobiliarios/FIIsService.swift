//
//  FIIsService.swift
//  UpYou
//
//  Created by Lucas Neves dos santos pompeu on 14/10/23.
//

import Foundation

class FIIsService {
    
    func getFiisService(completion: @escaping (FIIsData?, Error?) -> Void) {
        let urlString: String = "https://run.mocky.io/v3/d2f0f3c2-5379-4b01-9a7c-ee34827468d7"
        
        guard let url: URL = URL(string: urlString) else { return completion(nil, ErrorDetail.errorURL(urlString: urlString))}
        
        var request = URLRequest(url: url)
        request.httpMethod = "GET"
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            
            guard let dataResult = data else { return completion(nil, ErrorDetail.detailError(detail: "Error Data"))}
            
            let json = try? JSONSerialization.jsonObject(with: dataResult, options: [])
            print(json as Any)
            
            guard let response = response as? HTTPURLResponse else { return }
            
            if response.statusCode == 200 {
                do {
                    let fiisData: FIIsData = try JSONDecoder().decode(FIIsData.self, from: dataResult)
                    print("SUCCESS -> \(#function)")
                    completion(fiisData, nil)
                } catch  {
                    print("ERROR -> \(#function)")
                    completion(nil, error)
                }
            } else {
                print("ERROR -> \(#function)")
                completion(nil, error)
            }
        }
        task.resume()
    }
}
